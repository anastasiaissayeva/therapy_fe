<template>
  <div class="sidebar">
    ...
    <table class="styled-table">
      ...
      <tbody>
        <node-component v-for="item in items" :key="item.id" :id='item.id' :short_name="item.short_name"
    ...
    <node-add-form v-if="showAddForm" @add_item="addItem" @cancel_add="cancelAdd" />
...
import NodeAddForm from './NodeAddForm.vue';
export default {
  components: {
    NodeComponent,
    NodeAddForm,
  ...
  methods: {
    ...
    async getData() {
      ...
      try {
        ...
        const response = await this.$http.get(`node/?${params.toString()}`,
    ...
    async editData(id, short_name, name, note) {
      try {
        const response = await this.$http.put(`node/${id}/`, {
    ...
    async deleteData(id) {
      try {
        ...
        await this.$http.delete("node/" + id + '/', {
          headers: {
    ...
    async addItem(item) {
      ...
      try {
        ...
        await this.$http.post("node/", newItem, {
          headers: {
            authorization: `Bearer ${localStorage.access_token}`,
          },
        });
