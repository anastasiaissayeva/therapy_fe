<template>
  ...
  <tbody>
    ...
    <td>
      ...
      <td>
        <ul>
          <li v-for="lowerValue in ClinicalCase.datasets_result_lower_value" :key="lowerValue">{{ lowerValue }}</li>
        </ul>
      </td>
      <td>
        <ul>
          <li v-for="upperValue in ClinicalCase.datasets_result_upper_value" :key="upperValue">{{ upperValue }}</li>
        </ul>
      </td>
      <td>
        <ul>
...
<style scoped>
  ...
  .results {
    flex: 5; /* Занимает частей от общего пространства */
  ...
  .search-button:hover {
    background-color: var(--dark-green); /* Темнее при наведении */
  }
  ...
  .results li {
    ...
    border-bottom: 1px solid #eee; /* линия между элементами */
  }
