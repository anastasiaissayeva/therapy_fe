<template>
  <div class="sidebar">
    ...
    <table class="styled-table">
      <thead>
        <tr>
          <th @click="sort('short_name')">Сокращенное название</th>
          <th @click="sort('name')">Полное название</th>
          <th @click="sort('name')">Описание</th>
          <th>Действия</th>
        </tr>
      </thead>
      <tbody>
        <metastasis-component v-for="item in items" :key="item.id" :id='item.id' :short_name="item.short_name"
          :name="item.name" :note="item.note" @edit_item="edit_item" @delete_item="confirmDelete(item.id)" />
      </tbody>
    ...
    <metastasis-add-form v-if="showAddForm" @add_item="addItem" @cancel_add="cancelAdd" />
...
import MetastasisComponent from './MetastasisComponent.vue';
import MetastasisAddForm from './MetastasisAddForm.vue';
export default {
  components: {
    MetastasisComponent,
    MetastasisAddForm,
  ...
  methods: {
    ...
    async getData() {
      ...
      try {
        ...
        const response = await this.$http.get(`metastasis/?${params.toString()}`,
    ...
    async editData(id, short_name, name, note) {
      try {
        const response = await this.$http.put(`metastasis/${id}/`, {
    ...
    edit_item(id, new_short_name, new_name, new_note) {
      this.editData(id, new_short_name, new_name, new_note);
      this.refresh();
    },
    ...
    async deleteData(id) {
      try {
        ...
        await this.$http.delete("metastasis/" + id + '/', {
          headers: {
    ...
    async addItem(item) {
      ...
      try {
        ...
        await this.$http.post("metastasis/", newItem, {
          headers: {
            authorization: `Bearer ${localStorage.access_token}`,
          },
        });
