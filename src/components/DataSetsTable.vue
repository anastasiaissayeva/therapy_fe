<template>
    <div class="sidebar">
        ...
        <input v-model="searchQuery" @input="onSearchChange"
            placeholder="Поиск...(название модели,название источника)" />
        <table class="styled-table">
            <thead>
                <tr>
                    <th @click="sort('result')">Результат</th>
                    <th @click="sort('clinical_case')">Клинический случай</th>
            ...
            <tbody>
                <data-sets-component v-for="item in items" :key="item.id" :id='item.id' :name_result="item.name_result"
                    :name_clinical_case="item.name_clinical_case" :name_source="item.name_source" :results="results"
                    :clinical_cases="clinical_cases" :sources="sources" :note="item.note" @edit_item="edit_item"
        ...
        <data-sets-add-form v-if="showAddForm" @add_item="addItem" @cancel_add="cancelAdd" :results="results"
            :clinical_cases="clinical_cases" :sources="sources" />
...
export default {
    ...
    methods: {
        ...
        async editData(id, name_result_id, name_clinical_case_id, name_source_id, note) {
            try {
        ...
        edit_item(id, new_result_id, new_clinical_case_id, new_source_id, new_note) {
            console.log('Editing item:', id, new_result_id, new_clinical_case_id, new_source_id, new_note);
            this.editData(id, new_result_id, new_clinical_case_id, new_source_id, new_note);
            this.refresh();
        },
        ...
        async addItem(item) {
            ...
            try {
                const newItem = {
                    ...
                    clinical_case: item.name_clinical_case,
