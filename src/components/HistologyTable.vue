<template>
  <div class="sidebar">
      ...
      <button @click="toggleAddForm" class="add-button">
          Добавить гистологическую классификаццию
      </button>

      <histology-add-form v-if="showAddForm"
          @add_item="addItem"
...
import HistologyComponent from './HistologyComponent.vue';
import HistologyAddForm from './HistologyAddForm.vue';
export default {
  components: {
    HistologyComponent,
    HistologyAddForm,
...
if (this.searchQuery) {
  ...
  if (this.sortKey) {
    params.append('ordering', (this.sortOrder === 'desc' ? '-' : '') + this.sortKey);
        ...
        const response = await this.$http.get(`histology/?${params.toString()}`,
      ...
      async editData(id, name, note) {
          try {
              const response = await this.$http.put(`histology/${id}/`, {
                  name: name,
                  note: note
              }, {
                  headers: { authorization: `Bearer ${localStorage.access_token}` },
      ...
      async deleteData(id) {
          try {
              ...
              await this.$http.delete("histology/" + id + '/', {
                  headers: {
      ...
      async addItem(item) {
          ...
          try {
              ...
              await this.$http.post("histology/", newItem, {
                  headers: {
                      authorization: `Bearer ${localStorage.access_token}`,
                  },
              });
