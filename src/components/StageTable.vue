<template>
  <div class="sidebar">
      ...
      <stage-add-form v-if="showAddForm"
          @add_item="addItem"
...
import StageAddForm from './StageAddForm.vue';
  ...
  methods: {
    ...
    async getData() {
      ...
      try {
        ...
        const response = await this.$http.get(`stage/?${params.toString()}`,
      ...
      async editData(id, name, note) {
          try {
              const response = await this.$http.put(`stage/${id}/`, {
                  name: name,
                  note: note
              }, {
                  headers: { authorization: `Bearer ${localStorage.access_token}` },
      ...
      async deleteData(id) {
          try {
              ...
              await this.$http.delete("stage/" + id + '/', {
                  headers: {
      ...
      async addItem(item) {
          ...
          try {
              ...
              await this.$http.post("stage/", newItem, {
                  headers: {
                      authorization: `Bearer ${localStorage.access_token}`,
                  },
              });
