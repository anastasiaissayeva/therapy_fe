<template>
  <div class="sidebar">
      ...
      <table class="styled-table" >
        <thead>
          <tr>
              <th @click="sort('code')">Код МКБ-10</th>
              <th @click="sort('description')">Описание диагноза</th>
      ...
      <diagnosis-add-form v-if="showAddForm"
          @add_item="addItem"
...
import DiagnosisAddForm from './DiagnosisAddForm.vue';
import DiagnosisComponent from './DiagnosisComponent.vue';
export default {
  components: {
    DiagnosisComponent,
    DiagnosisAddForm,
  ...
  methods: {
    async getData() {
      ...
      try {
        ...
        const response = await this.$http.get(`diagnosis/?${params.toString()}`,
      ...
      async editData(id, code, description, note) {
          try {
              const response = await this.$http.put(`diagnosis/${id}/`, {
                  code: code,
      ...
      edit_item(id, new_code, new_description, new_note) {
        this.editData(id, new_code, new_description, new_note);
        this.refresh();
      },
      ...
      async deleteData(id) {
          try {
              ...
              await this.$http.delete("diagnosis/" + id + '/', {
                  headers: {
      ...
      async addItem(item) {
          ...
          try {
              ...
              await this.$http.post("diagnosis/", newItem, {
                  headers: {
                      authorization: `Bearer ${localStorage.access_token}`,
                  },
              });
