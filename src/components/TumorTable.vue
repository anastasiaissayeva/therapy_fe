<template>
  <div class="sidebar">
    ...
    <table class="styled-table">
      ...
      <tbody>
        <tumor-component v-for="item in items" :key="item.id" :id='item.id' :short_name="item.short_name"
    ...
    <button @click="toggleAddForm" class="add-button">
      Добавить вид опухоли
    </button>

    <tumor-add-form v-if="showAddForm" @add_item="addItem" @cancel_add="cancelAdd" />
...
import TumorComponent from './TumorComponent.vue';
import TumorAddForm from './TumorAddForm.vue';
export default {
  components: {
    TumorComponent,
    TumorAddForm,
  ...
  methods: {
    ...
    async getData() {
      ...
      try {
        ...
        const response = await this.$http.get(`tumor/?${params.toString()}`,
    ...
    async editData(id, short_name, name, note) {
      try {
        const response = await this.$http.put(`tumor/${id}/`, {
    ...
    async deleteData(id) {
      try {
        ...
        await this.$http.delete("tumor/" + id + '/', {
          headers: {
    ...
    async addItem(item) {
      ...
      try {
        ...
        await this.$http.post("tumor/", newItem, {
          headers: {
            authorization: `Bearer ${localStorage.access_token}`,
          },
        });
